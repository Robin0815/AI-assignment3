...
def breadth_first_search(maze_map, start_pos, goal_pos):
    ...
    -------
    [maze_map]
        [modded_maze_map with drawn line fitting to the algorythm: bfs]
    """
    ...
    while queue.__len__() > 0:
        current_node = queue.popleft() #only difference to dfs in this implementation, FIFO
...
if __name__ == '__main__':
    ...
    #sys.stdout=open(os.path.join(working_directory, 'results' + '/BFSMap1.txt'),"w", encoding='utf-8')
    ...
    #sys.stdout=open(os.path.join(working_directory, 'results' + '/BFSMap2.txt'),"w", encoding='utf-8')
    start_pos_map2 = get_start(maze_map_map2)
    goal_pos_map2 =get_goal(maze_map_map2)
    for goal_pos in goal_pos_map2:

        path_map2 = breadth_first_search(maze_map_map2, start_pos_map2, goal_pos)
        print(goal_pos)
        print_map(path_map2)
    ...
    #sys.stdout=open(os.path.join(working_directory, 'results' + '/BFSMap3.txt'),"w", encoding='utf-8')
    start_pos_map3 = get_start(maze_map_map3)
    goal_pos_map3 =get_goal(maze_map_map3)
    for goal_pos in goal_pos_map3:

        path_map3 = breadth_first_search(maze_map_map3, start_pos_map3, goal_pos)
        print(goal_pos)
        print_map(path_map3)
